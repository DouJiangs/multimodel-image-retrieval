...
def main():
    ...
    if opt.image_feat_type is not None:
        ...
        for i in range(image_num):
            ...
            file_trains = opt.img_train_feat
            file_valids = opt.img_train_feat
            file_trains = file_trains.replace('k_1',s)
            file_valids = file_valids.replace('k_1',s)
            trains = tables.open_file(file_trains,model='r')
            valids = tables.open_file(file_valids,model='r')
            train_item = trains.root.local_feats[:]
            valid_item = valids.root.local_feats[:]
            trains.close()
            valids.close()

            if flag == 0:
                flag = 1
                train_img_feats = train_item
                valid_img_feats = valid_item
            else :
                train_img_feats = np.concatenate((train_img_feats,train_item),axis=1)
                valid_img_feats = np.concatenate((valid_img_feats,valid_item),axis=1)
