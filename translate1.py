...
def main():
    ...
    if opt.image_feat_type is not None:
        ...
        for i in range(image_num):
            ...
            file_tests = opt.img_test_feat
            file_tests = file_tests.replace('k_1',s)
            tests = tables.open_file(file_tests,model='r')
            tests_item = tests.root.local_feats[:]
            tests.close()

            if flag==0:
                test_img_feats = tests_item
                flag=1
            else :
                test_img_feats = np.concatenate((test_img_feats,tests_item),axis=1)
