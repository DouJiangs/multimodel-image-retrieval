...
class Beam(object):
    ...
    def advance(self, word_probs, attn_out,attn_out_img=None, attn_out2=None):
        """
        ...
        * `attn_out`- attention at the last step
        ...
        if self.image_feat_type is not None:
            self.attn_img.append(attn_out_img.index_select(0, prev_k))
    ...
    def get_hyp(self, timestep, k):
        ...
        """
        hyp, attn, attn_img, attn2 = [], [], [], []
        for j in range(len(self.prev_ks[:timestep]) - 1, -1, -1):
            hyp.append(self.next_ys[j+1][k])
            attn.append(self.attn[j][k])
            if self.image_feat_type is not None:
                attn_img.append(self.attn_img[j][k])
                attn2.append(self.attn2[j][k])
            k = self.prev_ks[j][k]
        if self.image_feat_type is not None:
            return hyp[::-1], torch.stack(attn[::-1]), torch.stack(attn_img[::-1]), torch.stack(attn2[::-1])
