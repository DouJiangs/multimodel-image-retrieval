...
class RNNDecoderBase(nn.Module):
    ...
    def forward(self, tgt, memory_bank, state, memory_lengths=None, context_img=None):
        ...
        if context_img is not None and self.bi_attention:
            ...
            #text = text[:,[-1] ,:]
            #text = texts[:,[0],:]
            #for i in range(1,texts.size(1)):
            #    text = text + texts[:,[i],:]
            #text = text/texts.size(1)
            texts = texts.unsqueeze(1)
            texts = texts.repeat(1, 196, 1, 1)
            attention = torch.matmul(texts,image_atten)/np.sqrt(512)
            a = self.my_softmax(attention)
            a = torch.matmul(a,image)
            text = a[:,:,[0],:]
            ...
            text = memory_bank
            text = text[:,[-1],:]
            text = text.reshape(-1,1,1,512)
            text = text.repeat(1,196,num,1)
            atten = torch.cat((image,text),dim=3)
            atten = nn.Linear(512*2,512).cuda()(atten)
            atten = torch.tanh(atten)
            atten = nn.Linear(512,1).cuda()(atten)
            atten = atten.squeeze(3)
            score = nn.Softmax(dim=-1).cuda()(atten)
            attn = torch.matmul(score.unsqueeze(2),image).squeeze(2)
            context_img = attn
            '''

            # make new memory bank
            # context_img_cope = 
